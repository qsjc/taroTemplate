...
Component({
  ...
  deriveDataFromProps(nextProps) { // 组件创建时触发或更新时触发
    console.log(nextProps.palette, this.props, "组件创建")
      ...
      this.setData(
        ...
        () => {
          ...
          this.downloadImages(nextProps.palette).then(async palette => {
            ...
            if (needScale) {
              // NOTE 如果dpi超过2 那么就置为2
              const scale = getApp().systemInfo.pixelRatio > 2 ? 2 : getApp().systemInfo.pixelRatio;
    ...
    getCanvasContext(use2D, id) {
      ...
      return new Promise(resolve => {
        if (use2D) {
          ...
          query
            .select(selectId)
            // NOTE taro现在返回的小程序不是基于基础库2.0以上的 这api用不了
