    ...
    my.downloadFile({
      ...
      success: function(res) {
        ...
        my.getFileInfo({
          ...
          fail: error => {
            // 文件大小信息获取失败，则此文件也不要进行存储
            console.error(`getFileInfo ${res.apFilePath} failed, ${JSON.stringify(error)}`);
...
function saveFile(key, newFileSize, apFilePath, isDataUrl = false) {
  return new Promise((resolve, reject) => {
    if (isDataUrl) {
      const totalSize = savedFiles[KEY_TOTAL_SIZE] ? savedFiles[KEY_TOTAL_SIZE] : 0;
      savedFiles[key] = {};
      savedFiles[key][KEY_PATH] = apFilePath;
    ...
    my.saveFile({
      ...
      fail: error => {
        ...
        // 由于 saveFile 成功后，res.apFilePath 处的文件会被移除，所以在存储未成功时，我们还是继续使用临时文件
        resolve(apFilePath);
